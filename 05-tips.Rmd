...
You've made some changes that didn't work out for whatever reason, and now want to return your code to an earlier state. There are two ways to do this: _reset_ and _revert_.
...
This option keeps all the changes you have made, but adds an extra commit with new changes that _reverse_ the previous changes.
...
The two commands will result in a different git history. With `reset`, any changes you roll back will not appear, giving a shorter history; with `revert`, everything that happened will recorded and the history will be longer. So if it's not necessary for anyone to know about the rolled back changes, `reset` is probably better; if you need a record of what happened then `revert` is probably better.
...
* If any of the changes you want to reverse have been pushed to a remote server, you should use `revert` so that it's clear to others that changes have been rolled back. Using `reset` on a shared history will create confusion becaus it will not be clear what has happened.
...
Using the **pop** command does _not_ commit your changes - you have simply moved uncommitted changes from one branch to another - and once you have "popped" your changes they will be removed from the stash.
...
The git stash can technically hold many different sets of changes at once, but it is _not_ good practice to store anything in the stash long-term. It is a temporary store and should only be used to quickly move changes around when needed.
...
```
where `<commit-hash-1>` is the earliest commit you want to copy and `<commit-hash-n>` is the most recent. Then check out the branch you accidentally committed them to and toll it back as described [here](#reset-revert).