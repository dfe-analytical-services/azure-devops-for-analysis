...
* <img src="www/images/chapter2_RStudio_gitpaneProjectSettings.png" alt="Project settings icon" height="20"/> Open the project settings window - this provides the option to revert files to the state at the last commit if you want to undo any recent changes;
...
To make a commit, review the changes in the diff pane by selecting each file in turn and click the Stage button in the Review changes menu bar (either with multiple/all files selected at once or eac file selected indivudually). Then add your commit message and click the `Commit` button below the Commit message entry box.
...
Also in the review changes window, you can revert a file to its state at the last commit made. Simply select the file(s) you wish to revert and click the revert button.
