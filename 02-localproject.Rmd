...
Within RStudio, you can choose to manage your repository using the git bash terminal as in the previous sections, use the RStudio GUI elements or use a combination of both. Below we highlight the key things you need to know to get going with a git repository within R.
...
There are three key areas within RStudio for accessing git functionality. The the Git Pane, the terminal and the RStudio menu bar.
...
The Git pane offers an excellent quick look at the status of your repository, whilst offering quick links for viewing or committing changes, pulling and pushing, viewing the history, accessing the project settings, and creating and switching branches. Effectively all the common things you need to do with `Git`. The image below shows the git pane opened for a local clone of the repo for this guidance. It conssists of it's own menu bar with buttons for each of the options listed above, alongside a preview pane showing any files that have been modified, added or removed since your last commit.
...
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneDiff.PNG)  Open the [RStudio review changes window](#branches,-committing,-pulling-and-pushing)
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneCommit.png) Open the [RStudio review changes window](#branches,-committing,-pulling-and-pushing) (no this is not a typo that this does the same as the Diff button!)
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpanePull.png) Perform a pull
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpanePush.png) Push the current commits
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneHistory.png) Open the [RStudio review changes window](#branches,-committing,-pulling-and-pushing), defaulting to the History panel
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneProjectSettings.png) Open the project settings window
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneNewBranch.png) Create a new branch
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneSelectBranch.png) Shows the current branch and allows the user to switch branches via the drop down list
* ![RStudio Diff Icon](www/images/chapter2_RStudio_gitpaneRefresh.png) Open the commit window

#### Menu bar access

To access the git functionality via the menu bar, navigate to `Tools` > `Version control`. You should then see the options `Commit...`, `Pull Branches`, `Push Branch`, `Hsitory` and `Project Setup...`

![RStudio - Git in the menu bar](www/images/chapter2_RStudio_MenuBarAccess.PNG)

#### Branches, committing, pushing and pushing

The bulk of any adding, committing and pushing activities using the RStudio GUI is done using the Review changes window. It consists of 3 panes, the top left shows a preview of all files that have been added, modified or deleted, the top right pane allows you to add a commit message and the last pane shows the git diff result for any file selected in the file preview pane.
...
Finally, you can just click the `Push` button above the commit message entry box to sync your changes to the remote repository.
